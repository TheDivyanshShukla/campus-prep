(function () {
    ...
    window.onload = function () {
        if (typeof django !== 'undefined' && django.jQuery) {
            (function ($) {
                var dataEl = document.getElementById('admin-doc-data');
                if (!dataEl) return;
                ...
                $('#close-progress-overlay').click(function () { $('#ai-global-progress-overlay').fadeOut(); });

                // --- PREVIEW RENDERER ---
                function renderAdminPreview() {
                    try {
                        var rawJson = $('#id_structured_data').val();
                        if (!rawJson || rawJson.trim() === 'null') {
                            $('#admin-live-preview').html('<em>No structured data generated yet.</em>');
                            return;
                        }
                        var data = JSON.parse(rawJson);
                        var docType = $('#id_document_type').val();
                        var html = '';

                        var renderRecreationBox = function (strategy, details, url) {
                            if (!strategy || !details) return '';
                            var color = strategy === 'CANVAS' ? '#f59e0b' : '#10b981';
                            return '' +
                        ...
                        if ((docType === 'PYQ' || docType === 'UNSOLVED_PYQ') && data.questions) {
                            ...
                            data.sections.forEach(function (s) {
                                var blocks = '';
                                (s.content_blocks || []).forEach(function (b) {
                                    if (b.type === 'text') {
                                        blocks += '<div class="math-target">' + marked.parse(b.content || '') + '</div>';
                                    } else {
                                        blocks += renderRecreationBox(b.image_strategy, b.image_details, b.recreated_image_url);
                                    }
                                });
                ...
                var currentStatus = config.parsingStatus;
